...
{
    class FALSEUNION_API Sprite
    {
    public:
        Sprite(); //Default constructor for sprite.
        Sprite(const Texture* texture, const Vector2& position, const Vector2& size, const float& rotation,
               const Colour* colour); //Constructor for sprite that intakes parameters to set important variables.
