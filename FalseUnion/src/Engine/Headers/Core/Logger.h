...
namespace FalseUnion
{
    ...
    public:
        ...
        static void initializeLogger(Logger* clientLogger);
        ...
        // logs a message when given the params, output is different depenant on them
        void logToClient(const int& errorLevel, const std::string& information);
        // logs a message to client given params, output is different dependent on them
        ...
        // logs info, output is different dependant on params
        void logWarn(const bool& isToClient, const std::string& information);
        // logs warn, output is different dependent on params
        void logError(const bool& isToClient, const std::string& information);
        // logs error, output is different dependant on params
        void logInfoToClient(const std::string& information);
        // logs info to client, takes in a param for the logs text.
        void logWarnToClient(const std::string& information);
        // logs warn to client, takes in a param for the logs text.
        void logErrorToClient(const std::string& information);
        // logs error to client, takes in a param for the logs text.
        void logInfoToEngine(const std::string& information);
        // logs info to engine, takes in a param for the logs text.
        void logWarnToEngine(const std::string& information);
        // logs warn to engine, takes in a param for the logs text.
        void logErrorToEngine(const std::string& information);
        // logs error to engine, takes in a param for the logs text.

    protected:
        Logger(); // default constructor for logger
        virtual ~Logger(); // default virtual destructor for logger, used cause inheritance.

    private:
        void errorLevelSwitch(const int& errorLevel, const std::string& information) const;
        // switch statement broken into method to reduce code, takes in error level and information
